...
node *make_tree(node *root, char (*tokens)[MINLEN], int *idx, int parentheses)//트리 만들기
{
	node *cur = root;
	node *new;
	node *saved_operator;
...
int is_typeStatement(char *str)//답안에 타입정의문이 있는지 확인 //시발하나도모름
{ 
	char *start;
	char str2[BUFLEN] = {0}; 
	char tmp[BUFLEN] = {0}; 
	...
	while(start[0] == ' ')//시작값에 아무것도 없으면 start에 1을 더함
		start += 1;

	if(strstr(str2, "gcc") != NULL)//문장에 gcc가 들어있다면
	{
		strncpy(tmp2, start, strlen("gcc"));//tmp2 문자열에 start문자열을 복사함(띄어쓰기가 필요하기 때문임)
	...
	for(i = 0; i < DATATYPE_SIZE; i++)
	{
		if(strstr(str2,datatype[i]) != NULL)//데이터타입리스트중에 str2와 일치하는 게 있으면
		{	
			strncpy(tmp, str2, strlen(datatype[i]));//tmp배열에 그 값을 저장
			strncpy(tmp2, start, strlen(datatype[i]));//tmp2배열엔 나머지 값을 저장
